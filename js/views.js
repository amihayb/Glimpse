(() => {
  ...
  const STORAGE_KEY = 'glimpse-views';
  ...
  function captureConfig() {
    ...
    const gridRows = (Glimpse.state && Glimpse.state.gridRows) || 2;
    const gridCols = (Glimpse.state && Glimpse.state.gridCols) || 1;
    const gridPattern = (Glimpse.state && Glimpse.state.gridPattern) || 'coupled';
    ...
    const operationsLog = [...((Glimpse.state && Glimpse.state.operationsLog) || [])];
  ...
  function replayOperations(ops) {
    ...
    const transforms = Glimpse.data && Glimpse.data.transforms;
    if (!transforms) return;

    ops.forEach(op => {
      if (!rows[op.source]) return;
      let result;
      switch (op.type) {
      ...
      if (result) {
        ...
        Glimpse.ui.addCheckbox(op.result);
        Glimpse.state.operationsLog.push(op);
      }
    });
  }
  ...
  function applyView(view) {
    ...
    if (Glimpse.actions && Glimpse.actions.setSubplotGrid) {
      Glimpse.actions.setSubplotGrid(view.gridRows, view.gridCols);
    }

    // Set pattern (coupled / independent)
    if (Glimpse.actions && Glimpse.actions.setPattern) {
      Glimpse.actions.setPattern(view.gridPattern || 'coupled');
    }
  ...
  Glimpse.views.toggle = toggle;
  Glimpse.views.saveNewView = saveNewView;
  Glimpse.views.exportJSON = exportJSON;
  Glimpse.views.renderButtons = renderButtons;
